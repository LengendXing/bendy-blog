![Untitled](JUC%E6%A0%B8%E5%BF%83%E7%AF%87(%E4%B8%89)%EF%BC%9ALockSupport%E4%B8%8E%E7%BA%BF%E7%A8%8B%E9%98%BB%E5%A1%9E/Untitled.png)

### **开胃菜：你知道为什么使用Lock加锁解锁的时候需要手动释放锁吗？**

这是因为LockSupport的park底层是通过Unsafe的park来阻塞当前线程，调用UnSafe的park之前，还得先调用setBlocker给当前线程设置blocker值，之后在调用UnSafe的park阻塞线程，所以说AQS底层可以通过Unsafe+LockSupport实现的CAS安全机制，所以说，基于Unsafe+LockSupport的都需要手动去释放锁，实际上就是为了手动去执行setBlocker。所以才会有，Lock在基于LockSupport+Unsafe实现锁机制的时候，需要手动释放锁的原因是底层使用的是LockSupport的setBlocker来标记线程的，手动释放需要手动去除标记。
...
- Thread.sleep()不会释放占有的锁，Object.wait()会释放占有的锁；这是因为Object.wait()对应的锁是监视器锁，线程的等待和唤醒是基于对象的监视器锁（也称为内置锁）实现的。
...
实际上，它在阻塞当前线程之前还干了两件事，一是把当前线程添加到条件队列中，二是“完全”释放锁，也就是让state状态变量变为0，然后才是调用LockSupport.park()阻塞当前线程。注意一点，用LockSupport.park()来实现阻塞当前线程，并不代表，调用LockSupport.park()需要去管是否释放锁，他只负责阻塞，Condition.await()会释放锁是在进入阻塞之前释放了锁，而不是调用LockSupport.park()来实现阻塞当前线程的时候释放的锁。
...
- Object.wait()方法需要在synchronized块中执行；如果`Object.wait()`方法不在`synchronized`代码块内使用，那么它将无法自动释放锁。这意味着其他线程将无法获取锁对象，也无法访问共享资源。这可能导致死锁或其他线程无法正常执行的问题。
...
<aside>
💡 有关JUC-LockSupport的细节的问题，欢迎您在底部评论区留言，一起交流~
