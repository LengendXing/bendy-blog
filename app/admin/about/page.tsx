...
export default function AboutAdminPage() {
  ...
  useEffect(() => { fetch("/api/about").then(r => r.json()).then(d => d.name && setData(d)) }, [])
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="font-mono text-sm uppercase tracking-widest mb-6">// About Page</h1>
      <div className="space-y-4">
        <div><label className="font-mono text-xs block mb-1">Name</label><Input value={data.name} onChange={e => setData(d => ({ ...d, name: e.target.value }))} /></div>
        ...
        <div><label className="font-mono text-xs block mb-1">Extended Content (Markdown)</label><Textarea value={data.markdown} onChange={e => setData(d => ({ ...d, markdown: e.target.value }))} rows={8} /></div>
        <div>
          ...
          <div className="flex gap-2 mt-2">
            ...
            <Button size="sm" variant="outline" onClick={addLink}>Add</Button>
          </div>
        </div>
