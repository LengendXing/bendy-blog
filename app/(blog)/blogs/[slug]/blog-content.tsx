  ...
  return (
    <div className="mb-3">
      ...
      <textarea
        ...
        className="w-full border-2 border-pixel-black dark:border-pixel-white bg-transparent px-3 py-2 text-xs font-body focus:outline-none resize-y min-h-[60px]"
        rows={2}
      />
      {imageUrl && (
      ...
      <div className="flex items-center gap-2 mt-1">
        <Button size="sm" onClick={submit} disabled={submitting || (!content.trim() && !imageUrl)}>
          {submitting ? "..." : t.post}
        </Button>
        <span className="text-xs text-pixel-gray-400 flex items-center gap-1"><ImageIcon className="w-3 h-3" />{t.pasteImage}</span>
...
function CommentItem({ comment, allComments, postId, onNewReply, depth = 0 }: {
  ...
  const replyToComment = comment.replyToId ? allComments.find(c => c.id === comment.replyToId) : null

  return (
    <div className={depth === 0 ? "border-b border-pixel-gray-200 dark:border-pixel-gray-800 pb-3 mb-3" : "ml-6 border-l-2 border-pixel-gray-200 dark:border-pixel-gray-800 pl-3 mb-2"}>
      <div className="flex items-center gap-2 mb-1">
        ...
        {replyToComment && (
          <span className="text-xs text-pixel-gray-400">→ @{replyToComment.user.githubUsername || replyToComment.user.name}</span>
        )}
        <time className="text-xs text-pixel-gray-400 font-body">{new Date(comment.createdAt).toLocaleString()}</time>
      </div>
      ...
      {showReply && (
        <div className="mt-2">
          <CommentBox
            ...
            replyToName={comment.user.githubUsername || comment.user.name || ""}
            onPosted={(c) => { onNewReply(c); setShowReply(false) }}
            onCancel={() => setShowReply(false)}
          />
        </div>
      )}
      {replies.map(r => (
        <CommentItem key={r.id} comment={r} allComments={allComments} postId={postId} onNewReply={onNewReply} depth={1} />
      ))}
    </div>
  )
}
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12 flex gap-8 flex-col lg:flex-row">
      ...
      <aside className="lg:w-80 shrink-0 border-t-2 lg:border-t-0 lg:border-l-2 border-pixel-black dark:border-pixel-white lg:pl-8 pt-8 lg:pt-0">
        <h2 className="font-mono text-xs uppercase tracking-widest mb-6">// {t.comments}</h2>
        {session ? (
        ...
        <div>
          {topLevelComments.map(c => (
            <CommentItem key={c.id} comment={c} allComments={comments} postId={post.id} onNewReply={c => setComments(prev => [...prev, c])} />
          ))}
