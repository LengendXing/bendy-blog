...
export default function BlogsPage() {
  ...
  useEffect(() => {
    fetch("/api/blog?published=true").then(r => r.json()).then(data => {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-16">
      <h1 className="font-mono text-lg uppercase tracking-widest mb-12">// {t.blogs}</h1>
      {posts.length === 0 && <p className="font-body text-pixel-gray-500">{t.noPostsYet}</p>}
      <div className="space-y-0">
        {posts.map((post: any, i: number) => (
          <Link key={post.slug} href={`/blogs/${post.slug}`} className="group block border-b-2 border-pixel-gray-200 dark:border-pixel-gray-800 py-5 hover:bg-pixel-gray-100 dark:hover:bg-pixel-gray-900 px-4 -mx-4 transition-colors">
            ...
            {post.description && <p className="font-body text-xs text-pixel-gray-500 mt-1 ml-8 truncate">{post.description}</p>}
          </Link>
        ))}
      </div>
    </div>
